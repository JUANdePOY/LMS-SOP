...
CREATE TABLE `alert_rules` (
  ...
  `lookback_days` int(11) DEFAULT NULL COMMENT 'Number of days to look back (e.g., 90 for no training in 90 days)',
...
CREATE TABLE `alerts` (
  ...
  `target_squadron_id` bigint(20) DEFAULT NULL,
  `target_area_id` bigint(20) DEFAULT NULL,
  `is_active` tinyint(1) NOT NULL DEFAULT 1,
...
CREATE TABLE `areas` (
  ...
  `description` text DEFAULT NULL,
  `geographic_boundary` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin DEFAULT NULL CHECK (json_valid(`geographic_boundary`)),
  `is_active` tinyint(1) NOT NULL DEFAULT 1,
  `created_at` datetime NOT NULL DEFAULT current_timestamp(),
  `updated_at` datetime NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`id`),
...
CREATE TABLE `attendance` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `reservist_id` bigint(20) NOT NULL,
  `training_id` bigint(20) NOT NULL,
  `event_type` enum('internal','external') NOT NULL DEFAULT 'internal',
  `external_training_id` bigint(20) DEFAULT NULL,
  `status` enum('present','absent','late','excused','pending') NOT NULL DEFAULT 'pending',
  `check_in_time` datetime DEFAULT NULL,
  `check_out_time` datetime DEFAULT NULL,
  `location_check_in` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin DEFAULT NULL CHECK (json_valid(`location_check_in`)),
  `qr_code_used` varchar(255) DEFAULT NULL,
  `scan_method` enum('qr_scanner','camera','manual') DEFAULT NULL,
...
CREATE TABLE `external_training_attachments` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `external_training_id` bigint(20) NOT NULL,
...
CREATE TABLE `external_training_attendance` (
  ...
  `reservist_id` bigint(20) DEFAULT NULL,
...
CREATE TABLE `external_trainings` (
  ...
  `instructor` varchar(200) DEFAULT NULL COMMENT 'Facilitator or instructor name',
  `squadron_limits` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin DEFAULT NULL COMMENT 'Per-squadron slot limits (array of {squadron_id, slot_limit})' CHECK (json_valid(`squadron_limits`)),
  `registration_fields` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin DEFAULT NULL CHECK (json_valid(`registration_fields`)),
  `created_at` datetime NOT NULL DEFAULT current_timestamp(),
  `updated_at` datetime NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`id`),
...
CREATE TABLE `report_participants` (
  `report_id` bigint(20) NOT NULL,
  `reservist_id` bigint(20) NOT NULL,
  `squadron_id` bigint(20) DEFAULT NULL,
...
CREATE TABLE `reports` (
  ...
  `title` varchar(500) NOT NULL,
  `event_type` enum('internal','external') NOT NULL DEFAULT 'internal',
  `event_source_id` bigint(20) DEFAULT NULL COMMENT 'FK to trainings.id or external_trainings.id',
...
CREATE TABLE `reservists` (
  ...
  `category` enum('1st Category','2nd Category','3rd Category') DEFAULT NULL,
  `date_enlisted` date DEFAULT NULL,
  `source_of_commission` enum('ROTC','BCMT','MOTC','Direct Commission') DEFAULT NULL,
...
CREATE TABLE `system_alerts` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `rule_id` bigint(20) DEFAULT NULL COMMENT 'FK to alert_rules, NULL for ad-hoc',
  ...
  `title` varchar(500) NOT NULL,
  `message` text NOT NULL,
  `entity_type` varchar(50) DEFAULT NULL COMMENT 'squadron, reservist, supply, training, issuance',
  `entity_id` bigint(20) DEFAULT NULL COMMENT 'ID of the entity this alert is about',
  `entity_name` varchar(500) DEFAULT NULL COMMENT 'Denormalized name for display',
  `squadron_id` bigint(20) DEFAULT NULL,
  `group_id` bigint(20) DEFAULT NULL,
  `arsen_id` bigint(20) DEFAULT NULL,
...
CREATE TABLE `user_role_history` (
  ...
  `old_scope_arsen_id` bigint(20) DEFAULT NULL,
  `new_scope_arsen_id` bigint(20) DEFAULT NULL,
  `old_scope_group_id` bigint(20) DEFAULT NULL,
  `new_scope_group_id` bigint(20) DEFAULT NULL,
  `old_scope_squadron_id` bigint(20) DEFAULT NULL,
  `new_scope_squadron_id` bigint(20) DEFAULT NULL,
...
CREATE TABLE `users` (
  ...
  `scope_arsen_id` bigint(20) DEFAULT NULL,
  `scope_group_id` bigint(20) DEFAULT NULL,
  `scope_squadron_id` bigint(20) DEFAULT NULL,
  `is_active` tinyint(1) NOT NULL DEFAULT 1,
...
--
-- Temporary table structure for view `v_arsen_readiness`
--
...
--
-- Temporary table structure for view `v_group_readiness`
--
...
--
-- Temporary table structure for view `v_overall_readiness`
--
...
--
-- Temporary table structure for view `v_reservist_readiness`
--
...
--
-- Temporary table structure for view `v_squadron_readiness`
--
...
--
-- Final view structure for view `v_arsen_readiness`
--
...
--
-- Final view structure for view `v_group_readiness`
--
...
--
-- Final view structure for view `v_overall_readiness`
--
...
/*!50001 SET collation_connection      = cp850_general_ci */;
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50001 VIEW `v_reservist_readiness` AS select `r`.`id` AS `reservist_id`,`r`.`first_name` AS `first_name`,`r`.`last_name` AS `last_name`,`r`.`service_number` AS `service_number`,`r`.`rank` AS `rank`,`r`.`is_active` AS `is_active`,`r`.`specialization` AS `specialization`,`ra`.`group_id` AS `group_id`,`ra`.`squadron_id` AS `squadron_id`,`g`.`arsen_id` AS `arsen_id`,coalesce((select round(100.0 * sum(case when `a`.`status` = 'present' then 1 else 0 end) / nullif(count(0),0),2) from ((`internal_training_participants` `itp` join `trainings` `t` on(`itp`.`training_id` = `t`.`id`)) left join `attendance` `a` on(`a`.`training_id` = `t`.`id` and `a`.`reservist_id` = `itp`.`reservist_id`)) where `itp`.`reservist_id` = `r`.`id` and `t`.`is_mandatory` = 1),0) AS `training_participation_pct`,coalesce((select round(100.0 * sum(case when `a`.`status` in ('present','late') then 1 else 0 end) / nullif(count(0),0),2) from `attendance` `a` where `a`.`reservist_id` = `r`.`id`),0) AS `attendance_rate_pct`,case when `r`.`is_active` = 1 then 100.00 else 0.00 end AS `active_status_pct`,round(coalesce((select 0.40 * 100.0 * sum(case when `a`.`status` = 'present' then 1 else 0 end) / nullif(count(0),0) from ((`internal_training_participants` `itp` join `trainings` `t` on(`itp`.`training_id` = `t`.`id`)) left join `attendance` `a` on(`a`.`training_id` = `t`.`id` and `a`.`reservist_id` = `itp`.`reservist_id`)) where `itp`.`reservist_id` = `r`.`id` and `t`.`is_mandatory` = 1),0) + coalesce((select 0.30 * 100.0 * sum(case when `a`.`status` in ('present','late') then 1 else 0 end) / nullif(count(0),0) from `attendance` `a` where `a`.`reservist_id` = `r`.`id`),0) + case when `r`.`is_active` = 1 then 30.00 else 0.00 end,2) AS `readiness_score` from ((`reservists` `r` left join `reservist_assignments` `ra` on(`r`.`id` = `ra`.`reservist_id` and `ra`.`is_primary` = 1)) left join `groups` `g` on(`ra`.`group_id` = `g`.`id`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `v_squadron_readiness`
--
...
/*!50001 SET collation_connection      = cp850_general_ci */;
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50001 VIEW `v_squadron_readiness` AS select `s`.`id` AS `squadron_id`,`s`.`name` AS `squadron_name`,`s`.`code` AS `squadron_code`,`s`.`group_id` AS `group_id`,`g`.`name` AS `group_name`,`g`.`arsen_id` AS `arsen_id`,`a`.`name` AS `arsen_name`,count(distinct `vr`.`reservist_id`) AS `total_reservists`,count(distinct case when `vr`.`is_active` = 1 then `vr`.`reservist_id` end) AS `active_reservists`,round(avg(`vr`.`readiness_score`),2) AS `avg_readiness_score`,round(avg(`vr`.`training_participation_pct`),2) AS `avg_training_participation`,round(avg(`vr`.`attendance_rate_pct`),2) AS `avg_attendance_rate`,round(avg(`vr`.`active_status_pct`),2) AS `avg_active_status`,count(distinct case when `vr`.`readiness_score` < 65 then `vr`.`reservist_id` end) AS `below_threshold_count` from (((`squadron` `s` join `groups` `g` on(`s`.`group_id` = `g`.`id`)) join `arsens` `a` on(`g`.`arsen_id` = `a`.`id`)) left join `v_reservist_readiness` `vr` on(`vr`.`squadron_id` = `s`.`id`)) group by `s`.`id`,`s`.`name`,`s`.`code`,`s`.`group_id`,`g`.`name`,`g`.`arsen_id`,`a`.`name` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2026-06-03  0:32:57
