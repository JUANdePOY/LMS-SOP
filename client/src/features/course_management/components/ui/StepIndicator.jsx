      ...
      {Array.from({ length: totalSteps }, (_, i) => i + 1).map((step, idx) => (
        <div key={step} className="flex items-center flex-1">
          <div className="flex flex-col items-center">
            ...
            {labels[idx] && (
              <span className={`text-[10px] mt-1 font-medium ${currentStep >= step ? 'text-blue-700 dark:text-blue-300' : 'text-neutral-500 dark:text-neutral-400'}`}>
                {labels[idx]}
              </span>
            )}
          </div>
          {idx < totalSteps - 1 && (
            <div className={`flex-1 h-0.5 mx-2 rounded-full ${currentStep > step ? 'bg-blue-600' : 'bg-neutral-200 dark:bg-neutral-700'}`} />
          )}
        </div>
      ))}
    </div>
