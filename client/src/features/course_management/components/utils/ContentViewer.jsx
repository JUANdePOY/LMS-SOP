...
export default function ContentViewer({ content }) {
    ...
    return (
      <div className="rounded-xl border border-[var(--border)] bg-white dark:bg-neutral-900 p-6">
        ...
        <div
          ...
          dangerouslySetInnerHTML={{ __html: resolveBodyImages((content.description || content.content || "").replace(/<p>\s*<\/p>/g, '').trim() || "No content available.") }}
        />
      </div>
    );
  }
  if (content.type === "document") {
    return (
      <div className="rounded-xl border border-[var(--border)] bg-white dark:bg-neutral-900 p-6">
        <h3 className="text-lg font-semibold mb-2">{content.title}</h3>
        <p className="text-sm text-neutral-600">{(content.description || "").replace(/<p>\s*<\/p>/g, '').trim()}</p>
  ...
  const cleanDescription = (content.description || "").replace(/<p>\s*<\/p>/g, '').trim();
  return (
    <div className="rounded-xl border border-[var(--border)] bg-white dark:bg-neutral-900 p-6">
      <h3 className="text-lg font-semibold mb-2">{content.title}</h3>
      {cleanDescription && <p className="text-sm text-neutral-600">{cleanDescription}</p>}
    </div>
  );
}
