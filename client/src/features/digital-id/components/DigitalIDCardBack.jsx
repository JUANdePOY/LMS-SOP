import { Landmark, Home, Phone, Mail, QrCode, ExternalLink } from "lucide-react"
...
export default function DigitalIDCardBack({ profile = {}, links = [] }) {
  ...
  return (
    <div className="relative flex h-full w-full flex-col rounded-3xl bg-white" style={{ clipPath: "inset(0 round 1.5rem)" }}>
      {/* frame image */}
      <img src="/back.png" alt="" className="absolute inset-0 h-full w-full object-cover" />

      <div className="relative z-10 flex flex-1 flex-col items-center justify-center gap-5 px-[2%] pt-[6%]">
        <div className="relative flex items-center justify-center p-2">
          ...
          <div className="flex h-[120px] w-[120px] flex-col items-center justify-center gap-3 text-neutral-400">
            ...
            <span className="text-center text-xs px-4">
              QR code configuration coming soon
            </span>
          </div>
        </div>
            ...
            {socialLinks.map((link) => {
              ...
              return (
                <a
                  ...
                  className={`inline-flex items-center gap-1 rounded-full border border-neutral-200 bg-white px-2.5 py-1 text-[clamp(0.62rem,2.3vw,0.7rem)] font-medium shadow-sm transition hover:-translate-y-0.5 hover:border-[var(--color-primary)]/30 cursor-pointer ${platform.className}`}
                >
                  <Icon size={12} />
                  {platform.name}
      ...
      <div className="relative z-10 flex flex-col gap-3.5 px-[8%] pb-[8%] pt-4">
        ...
        {profile.address && (
          <ContactRow icon={Home} label="Address" value={profile.address} placeholder="Your address" />
        )}
        {profile.email && (
          <ContactRow icon={Mail} label="Email" value={profile.email} placeholder="youremail@example.email" />
        )}
      </div>
    </div>
  )
