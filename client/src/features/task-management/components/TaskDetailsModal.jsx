...
import { Users, MessageSquare, Paperclip, BarChart3, X } from 'lucide-react';
...
export default function TaskDetailsModal({ taskId, open, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
      ...
      <div
        className="relative z-10 w-full max-w-4xl rounded-xl border border-[var(--border)] bg-[var(--bg-surface)] shadow-xl max-h-[90vh] flex flex-col"
      ...
      >
        ...
        <div className="flex items-start justify-between gap-4 p-5 sm:p-6 border-b border-[var(--border)] shrink-0">
          ...
          <button
            type="button"
            onClick={onClose}
            className="p-2 -mr-2 rounded-lg hover:bg-[var(--bg-hover)] text-[var(--text-muted)] transition-colors"
            aria-label="Close"
          >
            <X size={20} />
          </button>
        ...
        {/* Task details grid */}
        {task && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm px-5 sm:px-6 py-3 border-b border-[var(--border)]">
          ...
          {TABS.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium whitespace-nowrap transition-colors ${activeTab === tab.id ? 'border-b-2 border-blue-500 text-blue-600 dark:text-blue-400' : 'text-[var(--text-muted)] hover:text-[var(--text-primary)]'}`}
              >
        ...
        <div className="flex-1 overflow-y-auto px-5 sm:px-6 py-4">
          {loading && !task ? (
            <div className="text-center py-8 text-sm text-[var(--text-muted)]">Loading task...</div>
          ) : error && !task ? (
                  ...
                  <div className="flex gap-2">
                    <div className="flex-1">
                      <textarea
                        ...
                        rows={2}
                        className="w-full rounded-lg border border-[var(--border)] bg-[var(--bg-page)] px-3 py-2 text-sm outline-none focus:border-blue-500 placeholder:text-[var(--text-muted)]"
                      />
                      {commentError && <p className="text-xs text-red-500 mt-1">{commentError}</p>}
                    </div>
                    <button
                      onClick={handleCommentSubmit}
                      className="shrink-0 rounded-lg bg-neutral-900 dark:bg-neutral-100 px-4 py-2 text-xs font-medium text-white dark:text-neutral-900 hover:bg-neutral-800"
                    >
                      Send
                    </button>
                  </div>
