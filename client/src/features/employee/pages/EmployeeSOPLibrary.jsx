...
import { Grid, List, Search, Filter, SortAsc, SortDesc, X, SlidersHorizontal, FileText } from "lucide-react";
import { useEmployeeSops } from "../hooks/useEmployeeSops";
import EmployeeSOPCard from "../components/EmployeeSOPCard";
...
export default function EmployeeSOPLibrary() {
  ...
  const { sops, loading, error, pagination, refetch } = useEmployeeSops(queryParams);
  ...
  useEffect(() => {
    if (sops.length > 0) {
      const uniqueCats = Array.from(new Set(sops.map((s) => s.category_name).filter(Boolean)));
  ...
  }, [search, selectedCategories, sortField, sortDirection, syncUrl]);
  ...
  const handleSopClick = (sopId) => {
    navigate(`/my-learning/sops/${sopId}`);
  };
  ...
  const hasActiveFilters = Boolean(search || selectedCategories.length);
    ...
    <div className="w-full max-w-none space-y-5 sm:space-y-6">
      <div className="relative overflow-hidden rounded-2xl border border-neutral-200/80 dark:border-neutral-700/80 bg-gradient-to-br from-white via-neutral-50/80 to-neutral-100/80 dark:from-neutral-900 dark:via-neutral-800 dark:to-neutral-700 px-5 sm:px-6 py-4 sm:py-5 shadow-sm dark:shadow-none">
        ...
        <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center justify-between">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-neutral-900 dark:text-neutral-100 tracking-tight">SOP Library</h1>
            <p className="text-xs sm:text-sm text-neutral-500 dark:text-neutral-400 mt-0.5">
              Browse your assigned and enrolled standard operating procedures
            </p>
          </div>
        ...
        <div className="min-w-0 flex-1 space-y-6">
          ...
          {error && (
            <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-sm">
              <p className="font-medium text-red-800 dark:text-red-300">Failed to load SOPs</p>
          ...
          {!loading && !error && sops.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-sm font-semibold text-neutral-700 dark:text-neutral-300">No SOPs found</p>
              <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-1">
                {hasActiveFilters
                  ? "Try adjusting your filters or clearing them to see all SOPs"
                  : "No SOPs are assigned to you yet"}
              </p>
              {hasActiveFilters && (
          ...
          {!loading && !error && sops.length > 0 && (
            <>
              <p className="text-xs text-neutral-500 dark:text-neutral-400" aria-live="polite">
                {pagination.total} {pagination.total === 1 ? "SOP" : "SOPs"} found
              </p>
              {viewMode === "grid" ? (
                ...
                pagination.totalPages > 1 && (
                  <div className="flex items-center justify-between pt-4 border-t border-neutral-200 dark:border-neutral-700">
                    <p className="text-xs text-neutral-500 dark:text-neutral-400">
                      Showing {(pagination.page - 1) * pagination.limit + 1}–{Math.min(pagination.page * pagination.limit, pagination.total)} of {pagination.total} SOPs
                    </p>
