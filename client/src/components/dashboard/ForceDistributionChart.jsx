...
import { forceDistributionData } from "@/data/dashboardData";
 ...
 * Horizontal grouped bar chart: Active vs Standby per area.
 */
export default function ForceDistributionChart() {
  return (
      ...
      <div className="mb-4 overflow-x-auto">
        <table className="w-full text-xs">
          ...
          <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
            {forceDistributionData.map((row) => (
              <tr key={row.area} className="text-neutral-700 dark:text-neutral-300">
                <td className="py-1.5 font-medium">{row.area}</td>
                <td className="py-1.5 text-right">{row.total.toLocaleString()}</td>
                <td className="py-1.5 text-right text-indigo-600 dark:text-indigo-400">{row.active}</td>
                <td className="py-1.5 text-right text-neutral-400">{row.standby}</td>
              </tr>
            ))}
          </tbody>
        </table>
