...
async function fix() {
  ...
  const sql = `INSERT IGNORE INTO reservists (id, user_id, first_name, last_name, \`rank\`, service_number, date_of_birth, age, sex, is_active) VALUES
  ...
  const [count] = await conn.query('SELECT COUNT(*) as cnt FROM reservists');
  console.log('Reservists count:', count[0].cnt);
  
  await conn.query("UPDATE users SET password_hash = '$2b$12$0FM.BWfgZvVsaGBN9qf2COz7B7HpqnXfpQdpLgel58lZv3/tHiP2m' WHERE email = 'ADMIN-001'");
  
  const [users] = await conn.query('SELECT u.email, r.service_number FROM users u JOIN reservists r ON u.id = r.user_id LIMIT 3');
  console.log('Sample joins:', users);
